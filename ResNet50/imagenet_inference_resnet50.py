...
with tf.Graph().as_default():
    ...
    #image = tf.image.decode_jpeg(tf.read_file("../SampleData/school_bus.jpg"), channels=3)
    image = tf.image.decode_png(tf.read_file('../SampleData/poodle.png'), channels=3)
    #image = tf.image.decode_png(tf.read_file('../SampleData/weasel.png'), channels=3)
    ...
    with tf.Session() as sess:
        ...
        np_image, network_input, probabilities = sess.run([image,
                                                           processed_image,
                                                           probabilities])
        probabilities = np.squeeze(probabilities, (0,1,2))
